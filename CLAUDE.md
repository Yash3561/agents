...
- If `WA_MEDIA_CAROUSEL_TEMPLATE_NAME` is configured, the webhook attempts a Meta-approved media-card carousel only when the visible result count exactly matches `WA_MEDIA_CAROUSEL_CARD_COUNT` (default five) and every result has an image. It falls back to sequential CTA cards for other counts, missing templates, ineligible results, or template rejection.
