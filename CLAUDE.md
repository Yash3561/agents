...
- A real Meta delivery of the new template-free media carousel; its payload has been validated with Meta calls intercepted, but the live phone path still needs one real 2+ product test.
...
- Global Concierge uses a template-free WhatsApp media carousel whenever the visible result count is 2–10 and every card has an image plus an HTTPS seller checkout URL. The card count, content, and seller CTA are generated from the current request. If Meta rejects the carousel or a result is missing required media/URL data, the webhook falls back to sequential seller CTA cards. This requires an active customer-service conversation; outside that window, use an approved template or a new inbound message.
...
- The webhook sends the intent-selected number of Global Catalog offers (up to five) as a template-free media carousel when possible. It does not use the per-merchant carousel because that flow's quick replies create a local merchant cart; Global Concierge instead uses one seller checkout CTA per card. A failed/ineligible carousel falls back to sequential seller CTA messages.
...
15. **The first carousel design was over-coupled to Meta templates.**

   A template forces a fixed approved card structure and a single URL base, which is awkward when Global Catalog results belong to different sellers. The Global Concierge path now uses a template-free interactive carousel during an active customer-service conversation: each card has its own seller checkout URL, while the visible count is chosen from the customer’s request. If Meta rejects the payload or the conversation is outside the service window, the webhook falls back to sequential seller CTA messages. The old template sender and its env vars are intentionally no longer part of the MVP path.
...
5. No carousel template or Meta template submission is required for the dev-only MVP. After a customer sends a message, the webhook can send a runtime-generated 2–10 card media carousel with one HTTPS seller checkout CTA per card. Verify the phone is inside WhatsApp’s active customer-service window; if a carousel payload is rejected, the app automatically sends sequential seller CTA cards instead. The implementation keeps the per-merchant quick-reply carousel separate because its buttons are tied to local cart IDs.
